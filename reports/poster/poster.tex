...
\node[box] (full) {
    ...
    {\small train all backbone weights}
};

\node[box, below=of full] (lora) {
    \textbf{LoRA}\\
    {\small low-rank update $W=W_0+sBA$}
};

\node[smallbox, below left=1.25cm and 0.65cm of lora] (dora) {
    \textbf{DoRA}\\
    {\small magnitude / direction}
};

\node[smallbox, below right=1.25cm and 0.65cm of lora] (lorapro) {
    \textbf{LoRA-Pro}\\
    {\small gradient correction}
};

\draw[arrow] (full) -- node[right, text=mutedgray] {\small cheaper PEFT} (lora);
\draw[arrow] (lora) -- node[left, text=mutedgray] {\small parameterization} (dora);
\draw[arrow] (lora) -- node[right, text=mutedgray] {\small optimization} (lorapro);
