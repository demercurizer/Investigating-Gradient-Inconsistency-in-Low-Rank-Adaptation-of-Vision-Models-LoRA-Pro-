...
\node[text=mutedgray] at (0,-1.65) {low-rank equivalent-gradient family};

\coordinate (o) at (-4.35,-0.35);
\coordinate (gaend) at (-3.35,0.38);
\coordinate (gbend) at (-4.35,-0.03);
\coordinate (sum) at ($(gaend)+(gbend)-(o)$);
\coordinate (g) at ($(sum)+(0,2.55)$);

\draw[ga] (o) -- (gaend)
    node[pos=0.54, above, text=titleblue] {$sBg_A$};

\draw[gb] (gaend) -- (sum)
    node[pos=0.35, above, text=orange!85!black] {$sg_BA$};

\draw[sumgrad] (o) -- (sum)
    node[pos=0.78, below, text=mutedgray] {$\widetilde g=sBg_A+sg_BA$};

\fill[mutedgray] (sum) circle (1.5pt);

\draw[fullgrad] (sum) -- (g)
    node[pos=0.72, right, text=titleblue] {$g=\partial\mathcal{L}/\partial W$}
    node[pos=0.35, left, text=mutedgray] {\scriptsize mismatch};
...
\node[align=left, text=mutedgray] at (1.8,-3.05) {
    \textcolor{titleblue}{blue}: contribution from \(g_A\) and full gradient\\
    \textcolor{orange!85!black}{orange}: contribution from \(g_B\)\\
    \textcolor{mutedgray}{gray dashed}: induced equivalent gradient
};
...
\(\widetilde g=sBg_A+sg_BA\) inside the low-rank family. LoRA-Pro adjusts
