...
\node[frozen, above left=2.2cm and 0.7cm of x] (w0) {
    \textbf{Frozen}\\[-0.1em]
    pretrained weight\\
    $W_0$
};

\node[trainable, above right=1.4cm and 1.0cm of x] (A) {
    \textbf{Trainable}\\[-0.1em]
    $A \in \mathbb{R}^{r\times k}$
};

\node[trainable, above=0.65cm of A] (B) {
    \textbf{Trainable}\\[-0.1em]
    $B \in \mathbb{R}^{d\times r}$
};

\node[output, above=1.55cm of $(w0)!0.5!(B)$] (sum) {$+$};

\node[output, above=1.05cm of sum] (h) {
    $h = W_0x + sBAx$
};

\draw[arrow] (x) -- node[left, text=mutedgray] {$x$} (w0);
\draw[arrow] (w0) -- node[left, text=mutedgray] {$W_0x$} (sum);

\draw[loraarrow] (x) -- node[below right, text=mutedgray] {$x$} (A);
\draw[loraarrow] (A) -- node[right, text=mutedgray] {$Ax$} (B);
\draw[loraarrow] (B) -- node[right, text=mutedgray] {$sBAx$} (sum);

\draw[arrow] (sum) -- (h);

\draw[decorate, decoration={brace, amplitude=5pt}, draw=mutedgray]
    ($(A.north east)+(0.18,0.08)$) -- ($(A.north west)+(-0.18,0.08)$)
    node[midway, above=7pt, text=mutedgray] {$r$-dimensional bottleneck};

\node[align=left, text=mutedgray, font=\normalsize] at ($(x)+(0,-1.25)$) {
