...
\node[trainable] (A) at (5.6,2.8) {
    \textbf{Trainable}\\[-0.1em]
    $A \in \mathbb{R}^{r\times k}$
};

\node[trainable] (B) at (5.6,5.35) {
    \textbf{Trainable}\\[-0.1em]
    $B \in \mathbb{R}^{d\times r}$
};

\node[output] (sum) at (0,7.05) {$+$};

\node[output] (h) at (0,8.85) {
    $h = W_0x + sBAx$
};

\draw[arrow] (x) -- (w0);
\draw[arrow] (w0) -- node[above left, text=mutedgray, pos=0.55] {$W_0x$} (sum);

\draw[loraarrow] (x) -- (A);
\draw[loraarrow] (A) -- node[right, text=mutedgray, pos=0.5] {$Ax$} (B);
\draw[loraarrow] (B) -- node[above right, text=mutedgray, pos=0.55] {$sBAx$} (sum);
